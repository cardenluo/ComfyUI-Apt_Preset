...
class sch_split_text:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                # 预设分隔符选择，None 时使用自定义 delimiter
                "preset": (
                    ["None", "按行", "按空格", "按逗号", "按句号", "按分号", "按制表符", "按竖线"],{"default": "None", }
                ),
                # 自定义分隔符，preset 为 None 时生效
                "delimiter": ("STRING", {"default": " ", "label": "自定义分隔符", "tooltip": "preset 为 None 时生效，支持 \\n 表示换行"}),
            }
        }
    ...
    DESCRIPTION = """
    支持两种分隔方式：
    1. 选择预设：直接使用常用分隔符（按行/空格/逗号等）
    2. 自定义分隔符：preset 设为 None 时生效，支持 \\n 表示换行、\\t 表示制表符
    预设说明：
    - 按行：使用换行符 \\n 分隔
    - 按空格：使用空格  分隔（连续空格会合并为一个分隔符）
    - 按逗号：使用 , 分隔（自动忽略逗号前后的空白字符）
    - 按句号：使用 . 分隔
    - 按分号：使用 ; 分隔
    - 按制表符：使用 \\t 分隔
    - 按竖线：使用 | 分隔
    ...
    def text_to_list(self, text, current_frame, preset=None, delimiter=" "):
        ...
        if preset is not None:
            ...
            delimiter = preset_map[preset]
        
        # 处理分隔符的转义字符（如 \n -> 实际换行）
        delimiter = delimiter.replace("\\n", "\n").replace("\\t", "\t")
        
        # 特殊处理：按空格时，合并连续空格（避免空字符串元素）
        if delimiter == " ":
            # 先替换连续空格为单个空格，再去除首尾空格，最后分割
            import re
            text = re.sub(r'\s+', ' ', text.strip())
            strList = text.split(" ") if text else []
        # 特殊处理：按逗号时，忽略逗号前后的空白字符（如 "a, b, c" -> ["a", "b", "c"]）
        elif delimiter == ",":
            import re
            strList = re.split(r'\s*,\s*', text.strip())
            # 移除可能的空字符串（如开头/结尾有逗号的情况）
            strList = [item for item in strList if item]
        else:
            # 普通分隔符分割
            strList = text.split(delimiter)
            # 移除空字符串（避免分隔符在开头/结尾或连续出现导致的空元素）
            strList = [item for item in strList if item]
        
        # 处理索引越界情况
        if current_frame < 0 or current_frame >= len(strList):
            return ("", len(strList))
