...
class text_mul_Join:
    ...
    def smart_join(self, join_rule: str, custom_pattern: str, **kwargs):
        ...
        if join_rule == "Custom":
            ...
            # 预设规则模式：用对应分隔符合并非空文本，兼容中英文符号
            separator = self._get_join_separator(join_rule)
            if join_rule == "Comma":
                joined_text = "，".join(non_empty_texts) if non_empty_texts else ""
            elif join_rule == "Period":
                joined_text = "。".join(non_empty_texts) if non_empty_texts else ""
            elif join_rule == "Semicolon":
                joined_text = "；".join(non_empty_texts) if non_empty_texts else ""
            else:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "split_rule": (["None", "Line", "Space", "Comma", "Period", "Semicolon", "Tab", "Pipe", "Custom"], {"default": "Line"}),
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "filter_rule": (["None", "@text@", "@text", "text @", '"text"', "'text'", "{text}", "(text)", "custom"], {"default": "None"}),
...
WILDCARDS_LIST = (
    ["None"]
    + [
        f"dir1 | {str(wildcard.relative_to(wildcards_dir1))[:-4]}"
        for wildcard in wildcards_dir1.rglob("*.txt")
    ]
    + [
        f"base_path | {str(wildcard.relative_to(wildcards_dir2))[:-4]}"
...
class text_wildcards:
    ...
    def INPUT_TYPES(cls):
        inputs = {
            "required": {
                "output_join_rule": (
                    ["Line", "Space", "Comma", "Period", "Semicolon", "Tab", "Pipe", "Custom"],
                    {"default": "Comma"}
                ),
                "custom_join": ("STRING", {
                    "multiline": True,
                    "default": "{text0},{text1},{text2},{text3},{text4},{text5},{text6},{text7},{text8}",
                    "placeholder": "在Custom模式下，才有效It only works in Custom mode"
                }),
    ...
    随机通配符提示词合并工具
    功能：
    1. 支持text0（自定义文本）+ text1-text8（通配符文件）共9个输入项，text1-text8选None则跳过
    2. text0为自定义文本前缀，可直接输入任意内容；text1-text8从指定通配符文件按input_split_rule拆分后随机抽取一项
    3. 按output_join_rule合并所有有效内容，Custom模式支持在custom_join中用{text0}-{text8}占位符自由排版
    输入分割规则（input_split_rule）：
    Line：按行拆分 | Space：按空格拆分 | Comma：按逗号拆分 | Period：按句号拆分 | Semicolon：按分号拆分 | Tab：按制表符拆分 | Pipe：按竖线拆分 | Custom：自定义分隔符（填写custom_split）
    输出合并规则（output_join_rule）：
    Line：换行 | Space：空格 | Comma：逗号 | Period：句号 | Semicolon：分号 | Tab：制表符 | Pipe：竖线 | Custom：自定义模板（填写custom_join）
    """
    ...
    def _get_wildcard_content(self, wildcard_key: str, seed: int, input_split_rule: str, custom_split: str) -> str:
        ...
        wildcard_file = Path(target_dir) / f"{wildcard_filename}.txt"
        if not wildcard_file.is_file():
            print(f"⚠️ Wildcard File not found: {wildcard_file}")
            return ""
        ...
        try:
            ...
            if not split_lines or split_lines == [""]:
                print(f"⚠️ No valid content in wildcard file: {wildcard_file}")
                return ""
            
            random.seed(seed)
            return random.choice(split_lines)
        except Exception as e:
            print(f"⚠️ Read wildcard file failed: {wildcard_file}, error: {str(e)}")
            return ""

    def _replace_custom_placeholders(self, custom_join: str, text_dict: dict) -> str:
        normalized_pattern = self._normalize_text(custom_join)
        result = normalized_pattern
    ...
    def stack_Wildcards(self, output_join_rule: str, input_split_rule: str, custom_split: str, custom_join: str, seed: int, text0="", **kwargs):
        ...
        if output_join_rule == "Custom":
            ...
            if output_join_rule == "Comma":
                joined_content = "，".join(non_empty_texts) if non_empty_texts else ""
            elif output_join_rule == "Period":
                joined_content = "。".join(non_empty_texts) if non_empty_texts else ""
            elif output_join_rule == "Semicolon":
                joined_content = "；".join(non_empty_texts) if non_empty_texts else ""
            else:
                joined_content = separator.join(non_empty_texts) if non_empty_texts else ""
        
        final_result = joined_content if joined_content else "❌ No valid wildcard content"
        
        return (self._normalize_text(final_result),)
