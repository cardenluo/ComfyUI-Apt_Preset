...
class Image_crop_visual:
    ...
    滚动鼠标，缩放裁剪框
"""
    def crop_image(self, image, crop_width, crop_height, crop_state):
        ...
        out_list = []
        for img in image:
            out_list.append(_crop_visual_apply_single(img, crop_w, crop_h, cx, cy, zoom))
        ...
        ui = {
            "bg_image": bg_results,
            "crop_meta": [{"img_w": int(image.shape[2]), "img_h": int(image.shape[1])}],
        }
