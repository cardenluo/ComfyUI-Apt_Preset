...
class IO_save_image:
    ...
    def save_image(self, image, file_format, filename_mid="Apt", output_path="", naming_format="命名+序号", number_digits=5,
                   save_workflow_as_json=False, prompt=None, extra_pnginfo=None, file_names=None):
...
class IO_LoadTextBatch:
    ...
    def load_text_list(self, text_list: str, card_size: int = 120, index: int = 1, text_list_in=None, unique_id: str = ""):
        ...
        return (texts, str(texts[i - 1]), int(i))

    @classmethod
    def IS_CHANGED(s, text_list: str, card_size: int = 120, index: int = 1, text_list_in=None):
        m = hashlib.sha256()
    ...
    @classmethod
    def VALIDATE_INPUTS(s, text_list: str, card_size: int = 120, index: int = 1, text_list_in=None):
        return True
...
class IO_LoadImgBatch:
    ...
    def load_img_batch(self, image_list: str, card_size: int = 120, index: int = 1, image_list_in=None, unique_id: str = ""):
        ...
        output_image = output_images[i - 1] if i - 1 < len(output_images) else blank
    ...
    @classmethod
    def IS_CHANGED(s, image_list: str, card_size: int = 120, index: int = 1, image_list_in=None):
        m = hashlib.sha256()
    ...
    @classmethod
    def VALIDATE_INPUTS(s, image_list: str, card_size: int = 120, index: int = 1, image_list_in=None):
        return True
    ...
    def load_images(self, image_list: str, selected_indices: str, thumb_size: int = 64, import_image=None, import_path: str = "", unique_id: str = ""):
        ...
        try:
            ...
            if isinstance(selected, list):
                ints = [i for i in selected if isinstance(i, int)]
                one_based = 0 not in ints and any(i >= 1 for i in ints)
                if one_based:
                    ints = [i - 1 for i in ints]
                valid_indices = [i for i in ints if i >= 0 and i < len(names)]
                if valid_indices:
...
class IO_LoadShotBatch:
    ...
    def load_shot_list(
        ...
        return (output_image, str(output_text), int(i))
