...
class IO_store_image:
    ...
    RETURN_TYPES = ("IMAGE", "INT")  # 仅修改此处：新增INT类型的total输出
    RETURN_NAMES = ("image", "total")  # 对应命名，total存储数量
    FUNCTION = "store_image"
    CATEGORY = "Apt_Preset/View_IO"
    OUTPUT_NODE = True
    DESCRIPTION = """
    release_storage（清空原存储），注意：清空之前的图像，需要添加图片输入
    storage_all（存储所有）输入与最后一张存储图像相同，才添加。"""
    ...
    def store_image(self, image: Optional[torch.Tensor] = None, output_mode: bool = False, 
        ...
        if output_mode:
            ...
            # 确定输出图片：有存储的图片则使用存储的，否则使用输入图片（可能为None）
        ...
        total = len(GLOBAL_STORED_IMAGES)  # 存储的数量（GLOBAL_STORED_IMAGES的长度）
        
        if image_output in ("Hide", "Hide/Save"):
            return {"ui": {}, "result": (output_image, total)}  # 返回image和total
        return {"ui": {"images": results}, "result": (output_image, total)}  # 返回image和total
    ...
    @classmethod
    def IS_CHANGED(cls, image: Optional[torch.Tensor] = None, output_mode: bool = False) -> str:
        # 处理图片为None的情况
        img_id = f"{image.shape}-{id(image)}" if isinstance(image_tensor, torch.Tensor) else "none"
        return json.dumps({"image_id": img_id, "output_mode": output_mode})
...
class view_bridge_Text:    # web_node/view_Data_text.js
    ...
    RETURN_TYPES = ("STRING",)
    RETURN_NAMES = ("text",)
    INPUT_IS_LIST = (True, True, True)  # 新增布尔参数的列表支持
    OUTPUT_IS_LIST = (True,)
    OUTPUT_NODE = True
    ...
    def process(self, edit_mode=False, text="", display="", unique_id=None):
        # 处理列表输入的情况（取第一个值作为有效控制值）
        if isinstance(edit_mode, list) and edit_mode:
            use_simple = edit_mode[0]
        else:
            use_simple = edit_mode

        # 当启用简单输入模式时，使用display作为输入源，忽略text
        if use_simple:
            input_text = display
        else:
            input_text = text
