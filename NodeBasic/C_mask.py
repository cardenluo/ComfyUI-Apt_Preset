...
class Mask_Remove_bg:
    ...
    RETURN_TYPES = ("IMAGE", "MASK", "MASK", "IMAGE",)  # 新增4通道RGBA输出
    RETURN_NAMES = ("image", "mask", "invert_mask", "alpha_image",)  # 命名新增输出
    FUNCTION = "removebg"
    CATEGORY = "Apt_Preset/mask"
    ...
    def removebg(self, bg_img, image, threshold, rem_mode):
        ...
        mask_broadcast = mask_stack.permute(0, 2, 3, 1)
        ...
        # 生成4通道RGBA图像（3通道图像 + 1通道掩码作为Alpha）
        # 确保掩码与图像维度完全匹配
        rgba_alpha = mask_broadcast.repeat(1, 1, 1, 1)  # 保持单通道Alpha
        rgba_image = torch.cat([image2, rgba_alpha], dim=-1)  # 拼接为RGBA（最后一维从3→4）
        ...
        if rgba_image.shape[1] == 4:
            rgba_image = rgba_image.permute(0, 2, 3, 1)
        rgba_image = rgba_image.float()
        ...
        return (image2, mask_stack, invert_mask, rgba_image,)
