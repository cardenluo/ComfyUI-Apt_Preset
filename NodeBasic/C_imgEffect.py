...
class Image_Detail_HL_frequencye:
    ...
    def frequency_detail_restore(self, image, detail_image, keep_high_freq, erase_low_freq, mask_blur, blend_mode="linear light", blend_opacity=100, detail_strength=100, high_freq_threshold=0.5, mask=None, invert_mask=False):
        b_images = []
        for b in image:
...
def tensor_to_skia_image(img: torch.tensor) -> skia.Image:
    ...
    rgb = np.clip(np_img * 255.0, 0, 255).astype(np.uint8)
...
def skia_to_tensor(sk_img: skia.Image) -> torch.tensor:
    ...
    ret = torch.from_numpy(rgb_unpremul).unsqueeze(0)
    return ret
...
class lay_image_XYgrid(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        ret = io.Schema(
            node_id = "lay_image_XYgrid",
            display_name = "lay_image_XYgrid",
            category = "Apt_Preset/imgEffect",
            is_input_list = True,
            inputs=[
                io.Image.Input("images",  ),
                io.String.Input("row_labels", multiline=True, dynamic_prompts=True, placeholder="Data is split by lines \n1\n2\n3", optional=True,),

                io.String.Input("col_labels", multiline=True, dynamic_prompts=True, placeholder="Data is split by lines \nA\nB\nC", optional=True, ),
                io.Int.Input("gap", display_name="gap", default=0, min=0, max=128, ),
                io.Float.Input("font_size", display_name="font_size", default=50, min=6, max=1000, ),
    ...
    @classmethod
    def execute(self, images: Iterable[torch.tensor], row_labels: list = [], col_labels: list = [], gap: list[int] = [0], font_size: list[float] = [6], order: list[bool] = ["outside-in"], output_is_list: list[bool] = [False]) -> io.NodeOutput:
        FONT_SIZE_MIN = 6
        PADDING = 18
        ...
        output_is_list = output_is_list[0] if isinstance(output_is_list, list) else output_is_list
        
        # 将多行字符串分割成标签列表
        # 处理 ComfyUI 可能的输入格式：字符串或包装成列表的字符串
        def process_labels(labels_input):
            if not labels_input:
                return []
            if isinstance(labels_input, list):
                labels_text = labels_input[0] if labels_input else ""
            else:
                labels_text = str(labels_input)
            return [line.strip() for line in labels_text.strip().split('\n') if line.strip()] if labels_text else []
        
        row_labels_list = process_labels(row_labels)
        col_labels_list = process_labels(col_labels)
        images_flat = []
        if order: # outside-in
            for img in images:
        ...
        cols = max(1, len(col_labels_list))
        rows = max(1, len(row_labels_list))
        ...
        labels_cols_h = 0.0
        labels_data = []
        if len(col_labels_list) > 0:
            labels_data.append({
                "texts" : col_labels_list,
                "is_column" : True,
                "rotation" : 0,
