...
class create_Mask_match_shape2:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "bimg": ("IMAGE",),
                "bmask": ("MASK",),
                "b_color": (["black", "white", "red", "green", "blue", "gray"], {"default": "blue"}),
                "b_extrant_to_block": ("BOOLEAN", {"default": True}),
                "f_extrant_to_block": ("BOOLEAN", {"default": True}),
                "f_smoothness": ("INT", {"default": 1, "min": 0, "max": 150, "step": 1}),
                "f_opacity": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.05}),
    ...
    def compose(self, bimg, bmask, b_color, b_extrant_to_block, f_extrant_to_block, 
        ...
        b_x1, b_y1, b_w, b_h = 0, 0, bmask_width, bmask_height  # 初始化背景有效区矩形
        if contours:
        ...
        if has_foreground:
            ...
            # -------------------------- 核心修改：9宫格对齐逻辑 --------------------------
            # 1. 背景有效区关键对齐点（基于背景有效矩形 b_x1, b_y1, b_w, b_h）
            bg_points = {
                "center": (b_x1 + b_w//2, b_y1 + b_h//2),          # 中心（原有默认）
                "top_center": (b_x1 + b_w//2, b_y1),               # 顶边中心
                "bottom_center": (b_x1 + b_w//2, b_y1 + b_h),       # 底边中心
                "left_center": (b_x1, b_y1 + b_h//2),               # 左边中心
                "right_center": (b_x1 + b_w, b_y1 + b_h//2),        # 右边中心
                "top_left": (b_x1, b_y1),                           # 左上角
                "top_right": (b_x1 + b_w, b_y1),                    # 右上角
                "bottom_left": (b_x1, b_y1 + b_h),                  # 左下角
                "bottom_right": (b_x1 + b_w, b_y1 + b_h)            # 右下角
            }

            # 2. 前景有效区关键对齐点（基于前景有效矩形 f_x1, f_y1, f_w, f_h）
            fg_points = {
            ...
            # 3. 获取目标对齐点，计算前景粘贴坐标（核心：让前景目标点对齐背景目标点）
            target_bg_point = bg_points[align_mode]
            target_fg_point = fg_points[align_mode]
            base_x = target_bg_point[0] - target_fg_point[0] + f_x_offset  # X轴位置
            base_y = target_bg_point[1] - target_fg_point[1] + f_y_offset  # Y轴位置
            # --------------------------------------------------------------------------
