...
class Image_matchByMask:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "b_extrant_to_block": ("BOOLEAN", {"default": True}),
                "f_extrant_to_block": ("BOOLEAN", {"default": True}),
                # 新增包裹误差值参数，范围0-1，默认0.05（5%误差）
                "wrap_tolerance": ("FLOAT", {"default": 0.05, "min": 0.0, "max": 1.0, "step": 0.01}),
            },
            "optional": {
                "fimg": ("IMAGE",),
                "fmask": ("MASK",),
                # 仅保留"auto-in"和"auto-out"两种缩放模式
                "scale_mode": (["auto-in", "auto-out"], {"default": "auto-in"}),
                "image_output": (["Hide", "Preview", "Save", "Hide/Save"], {"default": "Preview"}),
            },
            "hidden": {"prompt": "PROMPT", "extra_pnginfo": "EXTRA_PNGINFO"},
        }
    ...
    def get_min_rect(self, mask_np):
        ...
        contours, _ = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not contours:
            return (mask_np.shape[1], mask_np.shape[0], 0, 0)  # 补充x,y默认值，避免报错
        largest_contour = max(contours, key=cv2.contourArea)
        x, y, w, h = cv2.boundingRect(largest_contour)
        return (w, h, x, y)  # 新增x,y返回值，用于计算实际边界


    def compose(self, bimg, bmask, b_color, b_extrant_to_block, f_extrant_to_block, wrap_tolerance,
                scale_mode, image_output,
                fimg=None, fmask=None,
                prompt=None, extra_pnginfo=None):
        ...
        if has_foreground:
            ...
            # 计算缩放比例（仅保留auto-in/auto-out逻辑）
            f_scale = 1.0  # 默认比例
            if scale_mode == "auto-in":  # 背景包含前景
                scale_w = b_real_width / W2
                scale_h = b_real_height / H2
                f_scale = min(scale_w, scale_h)
            elif scale_mode == "auto-out":  # 前景包含背景
                scale_w = b_real_width / W2
                scale_h = b_real_height / H2
                f_scale = max(scale_w, scale_h)
            
            # 适配前景矩形化处理（使用可控包裹误差值）
            if f_extrant_to_block and scale_mode in ["auto-in", "auto-out"]:
                # 根据wrap_tolerance调整缩放比例，误差值越大，缩放越小（预留空间越多）
                f_scale *= (1.0 - wrap_tolerance)
            
            f_scale = max(0.01, min(f_scale, 5.0))  # 限制缩放范围
            ...
            if f_contours:
                ...
                rf_center_x = f_x1 + f_w // 2
                rf_center_y = f_y1 + f_h // 2

            # 计算放置位置（中心对齐）
            base_x = b_center_x - rf_center_x
            base_y = b_center_y - rf_center_y
