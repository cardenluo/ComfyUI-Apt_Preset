...
class Image_transform_layer_adv:
    ...
    def process(self, x_offset, y_offset, rotation, scale, opacity, blending_mode, blend_strength,
                bj_img=None, fj_img=None, mask=None, coordinates="[]", coordinate_type="center", bg_fill="black"):
        # 颜色映射表
        color_mapping = {
            "black": (0, 0, 0),
            "white": (255, 255, 255),
        ...
        # ===================== 修复核心：坐标解析逻辑 =====================
        target_x, target_y = 0.5, 0.5  # 默认值
        try:
            # 解析用户输入的坐标字符串
            points = json.loads(coordinates.strip())
            if isinstance(points, list) and len(points) > 0:
                # 优先取第一个有效坐标点（不再限制index=1）
                for point in points:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "image": ("IMAGE",),
                "mask": ("MASK",),
                "smoothness": ("INT", {"default": 0, "min": 0, "max": 2000, "step": 1, "display": "slider"}),
