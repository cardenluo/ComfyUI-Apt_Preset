...
class Image_UpscaleModel:
    ...
    def INPUT_TYPES(s):
        ...
        return {"required":
                    {"image": ("IMAGE",),
                     ...
                     "upscale_pixel": (upscale_pixel_list, {"default": "bilinear" }),
    ...
    def apply_resize_image(cls, image: Image.Image, original_width, original_height, rounding_modulus, mode='scale', factor: float = 2.0, resize_long_side: int = 1024, resample='bicubic'):
        ...
        # 第二步：两种模式统一应用 rounding_modulus（关键修改）
        m = rounding_modulus
        if m != 0:
            # 向上取整到最近的模数倍数（保持原有逻辑）
    ...
    def upscale(self, image, upscale_model, rounding_modulus=8, pixel_mode="rescale", upscale_pixel="bilinear", pixel_rescale=2.0, pixel_resize_long_side=1024):
        if upscale_model == "None" and upscale_pixel == "None":
        ...
        for img in up_image:
            ...
            if upscale_pixel != "None":
                # 原有逻辑：执行像素缩放并应用rounding_modulus
                ...
                # 新增逻辑：upscale_pixel为None且upscale_model非None时，直接对模型输出图像应用rounding_modulus
                if upscale_model != "None":
                    m = rounding_modulus
                    if m != 0:
                        current_width, current_height = pil_img.size
                        # 对模型放大后的图像尺寸进行rounding_modulus处理
                        ...
                        # 保持原有采样方式（使用默认的bilinear避免失真）
                        resized_pil = pil_img.resize((new_width, new_height), Image.Resampling.BILINEAR)
                    else:
                        # rounding_modulus为0时不处理
                        resized_pil = pil_img
                else:
                    # upscale_model为None且upscale_pixel为None的情况已在开头排除，此处仅作兼容
                    resized_pil = pil_img
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "mask_min": ("FLOAT", {"default": 0.0, "min": -10.0, "max": 1.0, "step": 0.01}),
                "mask_max": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 10.0, "step": 0.01}),
                "extract_to_block": ("BOOLEAN", {"default": True}),  # 矩形化开关（与参考代码一致）
...
class Image_merge2image:
    ...
    def blend_images(self, image1, image2, image1_position, blendsmooth, mask2=None):
        ...
        if image1_position == "center":
            ...
            crop_y = max(0, (h2 - target_h) // 2)
            crop_x = max(0, (w2 - target_w) // 2)
            crop_y_end = min(crop_y + target_h, h2)
            crop_x_end = min(crop_x + target_w, w2)
            img2_cropped = img2[crop_y:crop_y_end, crop_x:crop_x_end, :]
            if img2_cropped.shape[0] < target_h or img2_cropped.shape[1] < target_w:
                img2_resized = np.zeros((target_h, target_w, 3), dtype=np.float32)
                offset_y = (target_h - img2_cropped.shape[0]) // 2
                offset_x = (target_w - img2_cropped.shape[1]) // 2
                img2_resized[offset_y:offset_y+img2_cropped.shape[0], 
                             offset_x:offset_x+img2_cropped.shape[1], :] = img2_cropped
            else:
                img2_resized = img2_cropped
            new_h2, new_w2 = target_h, target_w
        ...
        max_feather = min(blendsmooth, new_h2 // 2, new_w2 // 2)
        feather_range = max_feather if max_feather > 0 else 1

        base_mask = np.ones((new_h2, new_w2), dtype=np.float32)
        if image1_position == "center":
            try:
                from scipy.ndimage import distance_transform_edt
            except ImportError:
                print("Warning: scipy not found, using simple edge feather for center mode")
        ...
        if image1_position == "center":
            ...
            overlap = min(feather_range, w1, new_w2)
            total_width = w1 + new_w2 - overlap
            total_height = h1
            blended = np.zeros((total_height, total_width, 3), dtype=np.float32)
        else:
            overlap = min(feather_range, h1, new_h2)
            total_height = h1 + new_h2 - overlap
            total_width = w1
            blended = np.zeros((total_height, total_width, 3), dtype=np.float32)
        ...
        if image1_position == "center":
            ...
            start_x = w1 - min(feather_range, w1)
            ...
            start_x = total_width - w1 - new_w2 + min(feather_range, w1)
            start_x = max(0, start_x)
            end_x = start_x + new_w2
            end_x = min(total_width, end_x)
            background = blended[:, start_x:end_x, :]
            ...
            start_y = h1 - min(feather_range, h1)
            ...
            start_y = total_height - h1 - new_h2 + min(feather_range, h1)
            end_y = start_y + new_h2
            start_y = max(0, start_y)
            end_y = min(total_height, end_y)
            background = blended[start_y:end_y, :, :]
