...
# 阿里云Qwen API 基础配置（替换为官方最新endpoint，当前为通用版）
...
# 核心调用函数（原生HTTP，无SDK）
def analyze_text_no_sdk(model, api_key, final_prompt, max_tokens, seed=None, max_retries=3):
    if not api_key:
    ...
    # 构建Qwen API请求参数（和dashscope SDK参数对齐）
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json"
    }
    ...
    for retry in range(max_retries):
        try:
            # 原生HTTP请求（和豆包节点的requests.post逻辑一致）
            response = requests.post(
                QWEN_API_URL,
                headers=headers,
                data=json.dumps(payload),
                timeout=60,  # 超时时间和豆包节点对齐
                proxies={}   # 如需代理，参考豆包节点配置PROXIES
            )
...
# 核心调用函数（替换dashscope.MultiModalConversation，原生HTTP）
def analyze_images_no_sdk(images, model, api_key, final_prompt, max_tokens, seed=None, max_retries=3):
    if not api_key:
        raise ValueError("API密钥未配置，请通过以下方式之一设置：\n1. 设置系统环境变量 aliyun_API_KEY\n2. 在custom_nodes/ComfyUI-Apt_Preset/NodeExcel目录下创建ApiKey_AI_Qwen.txt并写入密钥")
    ...
    for retry in range(max_retries):
        try:
            # 原生HTTP POST请求（和豆包节点调用逻辑一致）
            response = requests.post(
                QWEN_MULTIMODAL_API_URL,
                headers=headers,
                data=json.dumps(payload),
                timeout=60,
                proxies={}  # 如需代理，参考豆包节点配置PROXIES
            ...
            # 解析多模态返回结果（兼容SDK输出格式）
            if "output" in result_json and "choices" in result_json["output"]:
                return result_json["output"]["choices"][0]["message"]["content"][0]["text"].strip()
            else:
                raise Exception(f"API返回格式异常: {json.dumps(result_json, indent=2)}")
