...
const AD_GUIDE_UI_VERSION = "2026.09.23-guide-v144-media-editor-initial-fit";
...
function currentQwen2WidgetValues(node) {
    return {
        ...
        output_size: Boolean(getWidgetValue(node, "output_size", false)),
...
function repairQwen2ConfiguredWidgetValues(node, info) {
    ...
    const validCurrentLayout = raw.length >= 8
        && typeof raw[0] === "string"
        && typeof raw[1] === "string"
        && typeof raw[2] === "string"
        && typeof raw[3] === "boolean"
        && Number.isFinite(Number(raw[4]))
        && typeof raw[5] === "boolean"
        && Number.isFinite(Number(raw[6]))
        && Number.isFinite(Number(raw[7]));
    ...
    if (stored && typeof stored === "object") {
        ...
        source = {
            ...
            ref_size_mode: raw[3], resolution: raw[4], output_size: raw[5],
            width: raw[6], height: raw[7],
        };
    } else if (legacyBrokenLayout) {
        source = {
            prompt: raw[0], stage_prompts: "[]", negative_prompt: raw[4],
            ref_size_mode: false, resolution: raw[3], output_size: false,
    ...
    const values = {
        ...
        output_size: source.output_size === true,
...
function installMediaEditorNode(nodeType, nodeData) {
    ...
    nodeType.prototype.connect = function connectMediaEditorStrict(slot, targetNode, targetSlot) {
        if (slot === 0 && targetNode && (targetNode.inputs || [])[targetSlot]) {
