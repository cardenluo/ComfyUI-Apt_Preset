...
function resequence(node) {
    const counts = { image: 0, video: 0, audio: 0, latent: 0, text: 0, batch: 0 };
...
function normalizeLinks(node, removeMissing = false) {
    ...
    for (const link of links) {
        ...
        if (!Number.isFinite(sourceId) || !["image", "video", "audio", "latent", "text", "batch"].includes(mediaType)) continue;
        if ((mediaType === "text" && (hasText || isMultiStage)) || (mediaType === "batch" && !isMultiStage)) continue;
...
function mediaLimits(node) {
    if (isFl2Target(node)) {
        return { image: MAX_MEDIA, video: 0, audio: 0, latent: 1, text: 0, batch: 1, total: MAX_MEDIA };
    }
    if (isMulTarget(node)) {
        return { image: MAX_MEDIA, video: MAX_MEDIA, audio: MAX_MEDIA, latent: 1, text: 0, batch: 1, total: MAX_MEDIA };
    }
...
function canAccept(node, mediaType) {
    ...
    const links = ensureLinks(node);
    const referenceCount = links.filter((link) => !["text", "latent", "batch"].includes(String(link.media_type || "image"))).length;
    if (!["text", "latent", "batch"].includes(mediaType) && referenceCount >= limits.total) return false;
...
function pruneLinksForMode(node) {
    ...
    for (const link of ensureLinks(node)) {
        ...
        const mediaCount = kept.filter((item) => !["text", "latent", "batch"].includes(String(item.media_type || "image"))).length;
        if (!limits[type] || counts[type] >= limits[type] || (!["text", "latent", "batch"].includes(type) && mediaCount >= limits.total)) continue;
        counts[type] += 1;
        kept.push(link);
    }
...
function patchGraphToPrompt() {
    ...
    app.graphToPrompt = async function graphToPromptWithOrderedMedia() {
        ...
        for (const node of app.graph?._nodes || []) {
            ...
            const transportLinks = orderedLinks.filter((link) => !["text", "batch"].includes(String(link.media_type || "")) && link !== carryLink);
...
function mentionOptions(node) {
    ...
    const orderedLinks = normalizeLinks(node).filter((link) => !["text", "latent", "batch"].includes(String(link.media_type || "image")));
...
function reorderMaterialLinks(node, sourceKey, targetKey, insertAfter = false) {
    ...
    if (links[from].media_type === "batch" || links[to].media_type === "batch") return false;
...
function refreshMaterialTray(node, suppliedOptions = null) {
    ...
    tray.textContent = "";
    const batchLink = normalizeLinks(node).find((link) => link.media_type === "batch");
    if (batchLink) tray.append(createBatchPromptCard(node, batchLink));
    resizeMaterialTray(node, options.length + (batchLink ? 1 : 0));
    if (!options.length && !batchLink) {
