...
function localizeNodeInstance(node) {
    ...
    const outputLabels = { positive: "positive", latent: "latent" };
    for (const output of node.outputs || []) {
        const key = String(output.name || "").toLowerCase();
        if (outputLabels[key]) setLocalizedSlotLabel(output, outputLabels[key]);
    }
...
function installQuickCreateCapture(canvas) {
    ...
    if (canvas === quickCreateCaptureCanvas && canvas.__adGuideEasyQuickCreateCaptureInstalledV10) return true;
    ...
    canvas.__adGuideEasyQuickCreateCaptureInstalledV10 = true;
    const handler = (event) => {
    ...
    quickCreateCaptureCleanup = () => {
        ...
        canvas.__adGuideEasyQuickCreateCaptureInstalledV10 = false;
...
function patchCanvas() {
    ...
    if (!canvas || canvas.__adGuideEasyCanvasPatchedV10 || typeof canvas.drawConnections !== "function") return;
    canvas.__adGuideEasyCanvasPatchedV10 = true;
    patchedCanvas = true;
...
function insertMaterialMentionAtCaret(node, option) {
    ...
    fragment.append(makeMentionChip(option));
...
function refreshMaterialTray(node, suppliedOptions = null) {
    ...
    if (!options.length) {
        ...
        empty.textContent = TEXT.materialEmpty;
        tray.append(empty);
        return;
    }
    for (const option of options) {
        ...
        card.addEventListener("dragstart", (event) => {
            node.__adGuideDraggedMaterialKey = card.dataset.materialKey;
            card.classList.add("is-dragging");
            event.dataTransfer?.setData?.("text/x-minimax-ad-guide-material", card.dataset.materialKey);
            if (event.dataTransfer) event.dataTransfer.effectAllowed = "move";
            event.stopPropagation();
        });
        card.addEventListener("dragend", (event) => {
            node.__adGuideDraggedMaterialKey = "";
        ...
        card.addEventListener("drop", (event) => {
            event.preventDefault();
            event.stopPropagation();
            const sourceKey = event.dataTransfer?.getData?.("text/x-minimax-ad-guide-material") || node.__adGuideDraggedMaterialKey || "";
            const bounds = card.getBoundingClientRect();
            reorderMaterialLinks(node, sourceKey, card.dataset.materialKey, event.clientX >= bounds.left + bounds.width / 2);
        });
...
function makeMentionChip(option) {
    ...
    chip.className = `ad-guide-mention-chip${option.pending ? " is-pending" : option.unresolved ? " is-unresolved" : ""}`;
...
function patchEditorKeyHandling() {
    const proto = globalThis.LGraphCanvas?.prototype;
    if (!proto || proto.__adGuidePromptKeyHandlingPatchedV10 || typeof proto.processKey !== "function") return;
    proto.__adGuidePromptKeyHandlingPatchedV10 = true;
...
function repairConfiguredWidgetValues(node, info) {
    ...
    const normalized = isLegacyGuideLayout ? {
        prompt,
...
function installNode(nodeType, nodeData) {
    ...
    if (nodeType.prototype.__adGuideEasyNodeInstalledV10) return;
    nodeType.prototype.__adGuideEasyNodeInstalledV10 = true;
    ...
    nodeType.prototype.onSerialize = function onSerializeH3Easy(info) {
        ...
        if (info) {
            info.widgets_values = [
                ...
                canonicalOption("ref_image_size", getWidgetValue(this, "ref_image_size", REF_IMAGE_MATCH)),
            ];
        }
        return result;
    };
...
function install() {
    ...
    style.textContent = `
      ...
      .ad-guide-material-card {
        display: grid; grid-template-rows: 42px 15px; flex: 0 0 54px; width: 54px; min-width: 54px; height: 62px; padding: 2px;
      ...
      .ad-guide-mention-chip.is-unresolved { color: #ff8822; text-decoration: underline wavy rgba(255,136,34,.86); text-decoration-thickness: 1px; }
