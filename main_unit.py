...
class pre_Unit_PromptWeight:
    ...
    def parse_with_auto_pack(self, prompt):
        ...
        for match in matches:
            ...
            if cleaned_prefix:
                ...
                weights.append(1.0)
                wrapped_parts.append(f"[{cleaned_prefix}@1]")
            ...
            wrapped_parts.append(f"[{anchor_content}@{anchor_weight}]")
        ...
        if cleaned_suffix:
            ...
            weights.append(1.0)
            wrapped_parts.append(f"[{cleaned_suffix}@1]")
        
        wrapped_str = '、'.join(wrapped_parts)
        
        return segments, weights, wrapped_str
    ...
    def fuse_conditions_equal(self, cond_features, cond_strengths):
        ...
        dtype = cond_features[0].dtype

        total_w = sum(cond_strengths)
        norm_w = [s / total_w if total_w > 1e-6 else 1.0/len(cond_strengths) for s in cond_strengths]

        max_len = max(cf.shape[0] for cf in cond_features)
        padded_feats = []
        for cf in cond_features:
            pad_len = max_len - cf.shape[0]
            pad = torch.zeros((pad_len, cf.shape[-1]), device=device, dtype=dtype) if pad_len>0 else None
            padded_feats.append(torch.cat([cf, pad], dim=0) if pad_len>0 else cf)
    ...
    def process(self, clip, prompt):
        prompt_info = ""
        segments, weights, wrapped_str = self.parse_with_auto_pack(prompt)

        if len(segments) == 0:
            tok_empty = clip.tokenize("")
            c_empty, p_empty = clip.encode_from_tokens(tok_empty, return_pooled=True)
            prompt_info = ""
        ...
        for seg in segments:
            ...
            cond_feat = c_out.squeeze(0)
            cond_features.append(cond_feat)

        fused_cond, fused_pooled, norm_weights = self.fuse_conditions_equal(cond_features, weights)
        ...
        final_weight_parts = [
            f"{seg}{int(round(norm_w * 100))}%"
            for seg, norm_w in zip(segments, norm_weights)
        ]
        prompt_info = "|".join(final_weight_parts)
        
        return ([[fused_cond, {"pooled_output": fused_pooled}]], prompt_info)
